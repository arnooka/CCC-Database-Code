...
public class Invoice {
	...
	public static void Invoices() throws FileNotFoundException {
		...
		for(int i = 0; i<o; i++){
			double subtotal =0, total = 0, endTotal = 0, fees = 0, taxes = 0, num = 0;
			int productCount = 0;
			...
			//Get the Customer/Salesperson name from the respective HashMap
			...
			for(int j = 0; j<products.length; j++){
				...
				String p[] = products[j].trim().split(":");
				//Equipment and Consultations rate multiplyer
				if(p.length == 2){
					String code = p[0];
					num = Double.parseDouble(p[1]);
		...
		//Print the remaining details for the Executive Report and print the Detailed Invoice Report
		System.out.println("===================================================================="
				 + "=====================================================================");
	...
	private static String invoiceString(String invoiceID, String customerCode, HashMap<String, Double> productMultiplyer, HashMap<String, Customer> customer,
		...
		//Something NEEDS to be done here for the format to work properly
		for(int i = 0; i<invoiceProducts.size(); i++){
			...
			if(customer.get(customerCode).getType().equals("C") && product.get(invoiceProducts.get(i)).getType().equals("C")){
				taxes += total*0.0425;
			}else if(customer.get(customerCode).getType().equals("C") && product.get(invoiceProducts.get(i)).getType().equals("L")){
				taxes += total*0.0425;
