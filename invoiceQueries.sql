...
select i.invoiceCode, p.productCode from invoices as i join invoiceProducts as p on i.invoiceID = p.invoiceID where i.invoiceCode = 'INV001';


##8. A query to get all the invoices of a particular customer
select invoiceCode, c.customerName from invoices as i join customers as c on i.customerID = c.customerID where c.customerCode = 'C001';
...
select p.personFirstName, p.personLastName, count(*) from invoices as i join persons as p on i.personID = p.personID group by i.personID;

##14. A query to find the total number of invoices that include a particular product
select p.productCode, count(*) as 'invoices containing product' from invoices as i join invoiceProducts as p on i.invoiceID = p.invoiceID where p.productCode = 'c3p0'; 
...
## This one will find any duplicates     
##select i.invoiceCode as 'invoice with conflicts', ip.productCode, count(*) as 'Times product appears' from invoices as i join  
	##invoiceProducts as ip on i.invoiceID = ip.invoiceID 
